...
def response(flow: http.HTTPFlow):
    global time_spent, points, lessons_complete, auto_answer, drip_feed
    url = flow.request.pretty_url
    ...
    if "api.bedrocklearning.org" in flow.request.pretty_url and "dashboard" in flow.request.pretty_url:
        try:
            if lessons_complete:
    ...
    if "https://api.bedrocklearning.org/api/students" and "dashboard" in flow.request.pretty_url:
        ...
        data["dripfeed"] = drip_feed
        data['dripfeedDays'] = drip_feed

        flow.response.content = json.dumps(data).encode('utf-8')

    if "https://api.bedrocklearning.org/api/school" and "name" in flow.request.pretty_url:
        response_text = flow.response.content.decode('utf-8')
        data = json.loads(response_text)
    ...
    if "https://api.bedrocklearning.org/api/students" and "history" in flow.request.pretty_url:
        response_text = flow.response.content.decode('utf-8')
        data = json.loads(response_text)
        for i in range(1, 100):
            blocks = ['Block 8', 'Block 9']
            selected_block = random.choice(blocks)
            random_date = f"{random.randint(2023, 2025)}-{random.randint(1, 12):02d}-{random.randint(1, 28):02d}T{random.randint(0, 23):02d}:{random.randint(0, 59):02d}:{random.randint(0, 59):02d}.{random.randint(0, 999):03d}"
            completed_lesson = {
                "learningType": "LESSON",
                "start": f"{random_date}",
                "finish": f"{random_date}",
                "score": 100.0,
                "blockName": f"{selected_block}",
                "topicName": f"{'B 8' if selected_block == 'Block 8' else 'Block 9'} T 1: Star Bedrock, Star",
                "learningName": f"Lesson {i}"
            }

            data['results'].append(completed_lesson)
        
        data['results'] = data['results'][:100]  
        data['totalCount'] = len(data['results'])
        data['totalPages'] = len(data['results']) / 20
...
def menu():
    ...
    while True:
        ...
        print(f"   → Complete Lessons  : {'Enabled' if lessons_complete == True else 'Disabled'}")
        print(f"   → Auto Answer  : {'Enabled' if auto_answer == True else 'Disabled'}")
        print(f"   → Dripfeed  : {drip_feed}")
        print()
        print_menu()
        try:
            ...
            if choice == 0:
                ...
                drip_feed = int(input("  Set Dripfeed: "))
            elif choice == 5:
                clear()
                break
            elif choice == 6:
